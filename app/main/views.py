...
from . import main
from .. import db
from .. models import User, Question, Telegram
...
def users():
    ...
    if request.method == 'POST':
        ...
        if not user:
            ...
            db.session.add(user)
            response_object['message'] = 'Пользователь добавлен в базу данных!'
        else: 
            response_object['message'] = 'Пользователь уже существует!'
        db.session.commit()
    else:
...
def telegrams():
    ...
    if request.method == "POST" and 'message' in request.json \
        ...
        user.telebot_check_user_exist(name, question, chat_id, message_id)
    else:
